...
const Card = ({ campaign, user }) => {
  ...
  return (
    <div
      ...
      className="w-full cursor-pointer bg-neutral-200 rounded-lg"
    >
      <img
        className="w-full place-content-center rounded-lg h-[210px] md:h-[300px] xl:h-[264px]"
        loading="lazy"
      ...
      <div className="p-2 sm:p-4">
        <h5 className="text-xl py-1  text-black font-bold truncate">{campaign.title}</h5>
        <p className="truncate pb-4 text-black">{campaign.description}</p>
        <div className="flex justify-between pb-4 gap-4">
          <div className="flex flex-col overflow-hidden">
            <span className="text-black">{campaign.collectedAmount}</span>
        ...
        {user === campaign.owner && getDaysLeft(campaign.deadline) > 0 ? (
          ...
          <div className="flex items-center">
            <Image
              className="bg-white rounded-full mr-2 p-2"
              src={Logo}
              alt="logo"
              width={36}
              height={36}
            ...
            <p className="text-sm text-black truncate">
              <span className="text-black mr-1">by</span> {campaign.owner}
            </p>
          </div>
        )}
        {isAlertOpen && (
